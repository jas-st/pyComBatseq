...
def pycombat_seq(
    ...
    glm_f = dge_obj.glmFit(design=design, dispersion=phi_matrix, 
                           prior_count=1e-4, alpha_reg=alpha_reg, lambda_reg=lambda_reg)
    ...
    if vci.input_type == "anndata":
        ...
        res.X = adjust_counts.T
        return res
    elif vci.input_type == "dataframe":
        return pd.DataFrame(adjust_counts, columns=list_samples, index=list_genes)
    else:
